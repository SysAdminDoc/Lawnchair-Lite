...
- Added five product screenshots captured from the signed release on an isolated Android emulator.
- Rebuilt the README around the install path, product value, privacy behavior, and compatibility.
- Updated Fastlane listing copy and GitHub discovery metadata.
- Retained the mint and charcoal icon after a visual brand review found it stronger than the replacement directions.
...
- Added swipe down dismissal for Settings.
- Made native app icons the default shape.
- Improved label contrast, drawer controls, and Settings transitions.
